...
`node_modules/` is generated by `npm install`, and `dist-electron/` is generated by the Electron build step used by `npm run dev` or `npm run build`. After cloning the repository, run the commands above to recreate them locally.
