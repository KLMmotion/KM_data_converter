...
def convert_dataframe_to_episode(
    ...
    *,
    ...
    for column in [*action_columns, *state_columns]:
        filtered_df = filtered_df.filter(dfn.col(column).is_not_null())
...
def _build_frame(
    *,
    ...
    frame["action"] = _build_vector_from_columns(
        data_columns=data_columns,
        columns=action_columns,
        row_idx=row_idx,
        expected_dim=action_dim,
    ...
    frame["observation.state"] = _build_vector_from_columns(
        data_columns=data_columns,
        columns=state_columns,
        row_idx=row_idx,
        expected_dim=state_dim,
...
def _build_vector_from_columns(
    *,
    data_columns: dict[str, list[object]],
    columns: list[str],
    row_idx: int,
    ...
    if len(columns) == 1:
        return to_float32_vector(data_columns[columns[0]][row_idx], expected_dim, label)

    parts = [to_float32_vector(data_columns[column][row_idx], -1, label) for column in columns]
    concatenated = np.concatenate(parts).astype(np.float32, copy=False)

    if expected_dim != -1 and concatenated.shape[0] != expected_dim:
        raise ValueError(f"{label} has dim {concatenated.shape[0]} but expected {expected_dim}.")
