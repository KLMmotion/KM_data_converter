...
export function LogConsole({ title, clearLabel, copyLabel, emptyLabel, logs, onClear }: LogConsoleProps) {
  ...
  return (
    <section className="flex h-[360px] min-h-[300px] flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-950/85 shadow-panel ring-1 ring-cyan-300/10">
      ...
      <div className="console-scroll min-h-0 flex-1 overflow-y-auto p-5 font-mono text-xs leading-6">
        {logs.length === 0 ? (
          <div className="flex h-full min-h-48 items-center justify-center rounded-xl border border-dashed border-white/10 text-slate-500">{emptyLabel}</div>
        ) : (
          logs.map((log) => (
            <pre
              key={log.id}
